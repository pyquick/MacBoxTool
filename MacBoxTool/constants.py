...
class Constants:
    def __init__(self):
        ...
        self.nightly_build:              str = "4654.1000"
        self.support_version:            str = "1.1.100039446canary" # prefix: unstable core. canary: very unstable
        # Kext Versioning
        ## Acidanthera
        ...
        self.qt_variant:          bool = False  # Determine if using wxPython variant
    ...
    def special_build(self):
        ...
        try:
            version.parse(self.macboxtool_version)
            return False
    ...
    @property
    def payload_path_iso(self):
        return self.original_path / Path("payloads.iso")
    ...
    def auto_patch_launch_agent_path(self):
        return self.launch_services_path / Path("com.pyquick.macboxtool.auto-patch.plist")

    @property
    def rsr_monitor_launch_daemon_path(self):
        return self.launch_services_path / Path("com.pyquick.macboxtool.rsr-monitor.plist")

    @property
    def update_launch_daemon_path(self):
        return self.launch_services_path / Path("com.pyquick.macboxtool.macos-update.plist")
    ...
    def config_template_path(self):
        """Config.plist template path in payloads/Config/"""
        return self.payload_path / Path("Config/config.plist")

    @property
    def config_backup_path(self):
        """Config backup path"""
        return self.payload_path / Path("Config/config-backup.plist")

    # EFI Builder Paths (for Windows build)
    @property
    def efi_builder_path(self):
        """EFI build output path in user home"""
        return Path.home() / ".macboxtool" / "efi_builder"
    ...
    @property
    def build_path(self):
        return self.current_path / Path("EFI_Build/")
