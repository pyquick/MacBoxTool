...
class KernelDebugKitMerge:
    ...
    def _merge_kdk(self, kdk_path: str) -> None:
        ...
        """
        logging.info("- Merging KDK with Root Volume: {kdk_name}".format(kdk_name=Path(kdk_path).name))
    ...
    def merge(self, save_hid_cs: bool = False) -> str:
        ...
        if kdk_obj.success is False:
            logging.info("Unable to get KDK info: {error_msg}".format(error_msg=kdk_obj.error_msg))
            raise Exception("Unable to get KDK info: {error_msg}".format(error_msg=kdk_obj.error_msg))
        ...
        if kdk_obj.kdk_already_installed is False:
            ...
            if not kdk_download_obj:
                logging.info("Could not retrieve KDK: {error_msg}".format(error_msg=kdk_obj.error_msg))
            ...
            if kdk_obj.validate_kdk_checksum() is False:
                logging.info("KDK checksum validation failed: {error_msg}".format(error_msg=kdk_obj.error_msg))
                raise Exception("KDK checksum validation failed: {error_msg}".format(error_msg=kdk_obj.error_msg))
            ...
            if kdk_obj.kdk_already_installed is False:
                ...
                logging.warning("KDK was not installed, but should have been: {error_msg}".format(error_msg=kdk_obj.error_msg))
                raise Exception("KDK was not installed, but should have been: {error_msg}".format(error_msg=kdk_obj.error_msg))
        ...
        logging.info("- Found KDK at: {kdk_path}".format(kdk_path=kdk_path))
