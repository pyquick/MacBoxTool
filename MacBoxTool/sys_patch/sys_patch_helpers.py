...
class SysPatchHelpers:
    ...
    def snb_board_id_patch(self, source_files_path: str):
        ...
        logging.info("Found unsupported Board ID {reported_board_id}, performing AppleIntelSNBGraphicsFB bin patching".format(reported_board_id=self.constants.computer.reported_board_id))
        ...
        logging.info("Replacing {board_to_patch} with {reported_board_id}".format(board_to_patch=board_to_patch, reported_board_id=self.constants.computer.reported_board_id))
        ...
        if len(board_to_patch_hex) > len(reported_board_hex):
            ...
            logging.info("Error: Board ID {reported_board_id} is longer than {board_to_patch}".format(reported_board_id=self.constants.computer.reported_board_id, board_to_patch=board_to_patch))
        ...
        if not Path(path).exists():
            logging.info("Error: Could not find {path}".format(path=path))
    ...
    def generate_patchset_plist(self, patchset: dict, file_name: str, kdk_used: Path, metallib_used: Path):
        ...
        data = {
            "MacBoxTool": f"v{self.constants.macboxtool_version}",
    ...
    def patch_gpu_compiler_libraries(self, mount_point: Union[str, Path]):
        ...
        On 13.3 with 3802 GPUs, MacBoxTool will downgrade GPUCompiler to resolve
        ...
        if not Path(DEST_DIR).exists():
            raise Exception("Failed to find GPUCompiler libraries at {dest_dir}".format(dest_dir=DEST_DIR))
