...
class MetallibList(ScrollArea):
    ...
    def _display_metallibs(self):
        ...
        if not self.available_metallibs:
    ...
    def _render_batch(self, start_index: int, batch_size: int = 20):
        ...
        end_index = min(start_index + batch_size, len(self.available_metallibs))

        for i in range(start_index, end_index):
            metallib = self.available_metallibs[i]
        ...
        if end_index < len(self.available_metallibs):
