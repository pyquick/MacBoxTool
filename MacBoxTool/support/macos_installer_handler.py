...
class InstallerCreation:
    ...
    def generate_installer_creation_script(self, tmp_location: str, installer_path: str, disk: str) -> bool:
        ...
        logging.info("Creating temporary directory at {0}".format(ia_tmp))
        ...
        if can_copy_on_write(installer_path, ia_tmp) is False:
            ...
            if space_available < space_needed:
                ...
                logging.info("{0} available, {1} required".format(utilities.human_fmt(space_available), utilities.human_fmt(space_needed)))
                return False
        ...
        if not Path(installer_path).exists():
            logging.info("Failed to copy installer to {0}".format(ia_tmp))
            return False
