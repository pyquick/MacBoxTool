...
from ..hardware import generate_smbios
...
class GenerateDefaults:
    ...
    def __init__(self, model: str, host_is_target: bool, global_constants: constants.Constants, ignore_settings_file: bool = False) -> None:
        ...
        self.global_settings = global_settings.GlobalSettings(self.constants)
    ...
    def _general_probe(self) -> None:
        ...
        if self.model in ["MacBookPro8,2", "MacBookPro8,3"]:
            ...
            ts2_status = self.global_settings.find_key("MacBookPro_TeraScale_2_Accel")
            if ts2_status is True:
                self.constants.allow_ts2_accel = True
            else:
                self.global_settings.settings["MacBookPro_TeraScale_2_Accel"] = False
                self.global_settings.save_settings()
        ...
        if self.global_settings.find_key("Force_Web_Drivers") is True:
            self.constants.force_nv_web = True

        result = self.global_settings.find_key("ShouldNukeKDKs")
        if result is False:
    ...
    def _load_gui_defaults(self) -> None:
        ...
        settings = self.global_settings.show_settings()
        ...
        for key, value in list(settings.items()):
            ...
            if hasattr(self.constants, constants_key):
                ...
                if original_type != new_type:
                    ...
                    self.global_settings.settings.pop(key, None)
                    self.global_settings.save_settings()
                    continue

                logging.info(f"{'Setting'} {constants_key} {'to'} {value}")
                setattr(self.constants, constants_key, value)