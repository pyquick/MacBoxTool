...
def verify(process_result: subprocess.CompletedProcess) -> None:
    ...
    raise Exception("process_failed_with_exit_code: {0}".format(process_result.returncode))
...
def generate_log(process: subprocess.CompletedProcess) -> str:
    ...
    output += "    Command: {0}".format(process.args)+"\n"
    output += "    Return Code: {0}".format(process.returncode)+"\n"
    _returned_error = __resolve_privileged_helper_errors(process.returncode)
    if _returned_error:
        output += "        Likely Enum: {0}".format(_returned_error)+"\n"
