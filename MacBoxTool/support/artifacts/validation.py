...
class PatcherValidation:
    ...
    def _build_prebuilt(self) -> None: 
        ...
        for model in model_array.SupportedSMBIOS:
            logging.info("Validating predefined model: {model}".format(model=model))
            ...
            if result.returncode != 0:
                ...
                raise Exception("Validation failed for predefined model: {model}".format(model=model))
            else:
                logging.info("Validation succeeded for predefined model: {model}".format(model=model))
    ...
    def _build_dumps(self) -> None:
        ...
        for model in self.valid_dumps:
            ...
            logging.info("Validating dumped model: {model}".format(model=self.constants.computer.real_model))
            build.BuildOpenCore(self.constants.computer.real_model, self.constants)
            result = subprocess.run([self.constants.ocvalidate_path, f"{self.constants.opencore_release_folder}/EFI/OC/config.plist"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            if result.returncode != 0:
                logging.info("Error on build!")
                subprocess_wrapper.log(result)
                raise Exception("Validation failed for predefined model: {model}".format(model=self.constants.computer.real_model))
            else:
                logging.info("Validation succeeded for predefined model: {model}".format(model=self.constants.computer.real_model))
    def _validate_root_patch_files(self, major_kernel: int, minor_kernel: int) -> None:
        ...
        for patch_core in patchset:
            ...
            for install_type in patchset[patch_core]:
                if install_type not in PatchType:
                    raise Exception("Unknown PatchType: {install_type}".format(install_type=install_type))
                    ...
                    for install_directory in patchset[patch_core][install_type]:
                        for install_file in patchset[patch_core][install_type][install_directory]:
                            ...
                            if not Path(source_file).exists():
                                logging.info("File not found: {source_file}".format(source_file=source_file))
                                raise Exception("Failed to find {source_file}".format(source_file=source_file))
        ...
        logging.info("Validating against Darwin {major_kernel}.{minor_kernel}".format(major_kernel=major_kernel, minor_kernel=minor_kernel))
        if not sys_patch_helpers.SysPatchHelpers(self.constants).generate_patchset_plist(patchset, f"MacBoxTool-{major_kernel}.{minor_kernel}.plist", None, None):
            raise Exception("Failed to generate patchset plist")

        # Remove the plist file after validation
        Path(self.constants.payload_path / f"MacBoxTool-{major_kernel}.{minor_kernel}.plist").unlink()
    ...
    def _validate_sys_patch(self) -> None:
        ...
        if not Path(self.constants.payload_local_binaries_root_path_dmg).exists():
            ...
            if dl_obj.download_complete is False:
                logging.info("Failed to download Universal-Binaries.dmg".format())
                raise Exception("Failed to download Universal-Binaries.dmg".format())

        logging.info("Validating Root Patch File integrity".format())
        ...
        if output.returncode != 0:
            logging.info("Failed to mount Universal-Binaries.dmg".format())
            subprocess_wrapper.log(output)

            raise Exception("Failed to mount Universal-Binaries.dmg".format())

        logging.info("Mounted Universal-Binaries.dmg".format())
        ...
        logging.info("Validating SNB Board ID patcher".format())
        ...
        if output.returncode != 0:
            logging.info("Failed to unmount Universal-Binaries.dmg".format())
            subprocess_wrapper.log(output)

            raise Exception("Failed to unmount Universal-Binaries.dmg".format())
