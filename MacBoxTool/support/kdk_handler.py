...
class KernelDebugKitObject:
    ...
    def _get_remote_kdks(self) -> list:
        ...
        try:
            ...
            results = network_handler.NetworkUtilities().get(
                KDK_API_LINK,
                headers={
                    "User-Agent": f"OCLP/{self.constants.mactoolbox_version}"
                },
                timeout=5
            )
    ...
    def _get_latest_kdk(self, host_build: str = None, host_version: str = None) -> None:
        ...
        if self.kdk_installed_path:
            logging.info("KDK already installed ({0}), skipping".format(Path(self.kdk_installed_path).name))
        ...
        if remote_kdk_version is None:
            ...
            logging.info("Checking for KDKs loosely matching {0}".format(loose_version))
            self.kdk_installed_path = self._local_kdk_installed(match=loose_version, check_version=True)
            if self.kdk_installed_path:
                logging.info("Found matching KDK: {0}".format(Path(self.kdk_installed_path).name))
                self.kdk_already_installed = True
                self.success = True
                return

            older_version = f"{parsed_version.major}.{parsed_version.minor - 1 if parsed_version.minor > 0 else 0}"
            logging.info("Checking for KDKs matching {0}".format(older_version))
            ...
            logging.warning("Couldn't find KDK matching {0} or {1}, please install one manually".format(host_version,host_build))

            self.error_msg = "Couldn't find KDK matching {0} ({1}) or {2} was installed.\nPlease ensure you have a network connection or manually install a KDK.".format(host_version,host_build,older_version)
        ...
        if self.kdk_url == "":
            if self.kdk_closest_match_url == "":
                logging.warning("No KDKs found for {0} ({1})".format(host_build, host_version))
                self.error_msg = "No KDKs found for {0} ({1})".format(host_build, host_version)
                return
            logging.info("No direct match found for {0}, falling back to closest match".format(host_build))
            logging.info("Closest Match: {0} ({1})".format(self.kdk_closest_match_url_build, self.kdk_closest_match_url_version))
            ...
            logging.info("Direct match found for {0} ({1})".format(host_build, host_version))
        ...
        logging.info("- KDK Build: {0}".format(self.kdk_url_build))
        logging.info("- KDK Version: {0}".format(self.kdk_url_version))
        logging.info("- KDK URL: {0}".format(self.kdk_url))
    ...
    def retrieve_download(self, override_path: str = "") -> network_handler.DownloadObject:
        ...
        if self.kdk_url == "":
            ...
            return None
        logging.info("Returning DownloadObject for KDK: {0}".format(Path(self.kdk_url).name))
        self.success = True
    ...
    def _generate_kdk_info_plist(self, plist_path: str) -> None:
        ...
        try:
            ...
            logging.error("Failed to generate KDK Info.plist: {0}".format(e))
    ...
    def _local_kdk_valid(self, kdk_path: Path) -> bool:
        ...
        if not Path(f"{kdk_path}/System/Library/CoreServices/SystemVersion.plist").exists():
            logging.info("Corrupted KDK found ({0}), removing due to missing SystemVersion.plist".format(kdk_path.name))
        ...
        if "ProductBuildVersion" not in kdk_plist_data:
            logging.info("Corrupted KDK found ({0}), removing due to missing ProductBuildVersion".format(kdk_path.name))
            self._remove_kdk(kdk_path)
            return False
        ...
        if result.returncode != 0:
            ...
            logging.info("pkg receipt missing for {0}, falling back to legacy validation".format(kdk_path.name))
        ...
        for line in result.stdout.decode("utf-8").splitlines():
            ...
            if not Path(f"{kdk_path}/{line}").exists():
                logging.info("Corrupted KDK found ({0}), removing due to missing file: {1}".format(kdk_path.name, line))
                self._remove_kdk(kdk_path)
                return False
    ...
    def _local_kdk_valid_legacy(self, kdk_path: Path) -> bool:
        ...
        for kext in KEXT_CATALOG:
            if not Path(f"{kdk_path}/System/Library/Extensions/{kext}").exists():
                logging.info("Corrupted KDK found, removing due to missing: {0}".format(f"{kdk_path}/System/Library/Extensions/{kext}"))
                self._remove_kdk(kdk_path)
                return False
    ...
    def _local_kdk_installed(self, match: str = None, check_version: bool = False) -> str:
        ...
        for kdk_pkg in Path(KDK_INSTALL_PATH).iterdir():
            ...
            logging.info("Found KDK backup: {0}".format(kdk_pkg.name))
    ...
    def _remove_kdk(self, kdk_path: str) -> None:
        ...
        if not Path(kdk_path).exists():
            logging.warning("KDK does not exist: {0}".format(kdk_path))
            return
...
class KernelDebugKitUtilities:
    ...
    def install_kdk_pkg(self, kdk_path: Path) -> bool:
        ...
        logging.info("Installing KDK package: {0}".format(kdk_path.name))
    ...
    def _create_backup(self, kdk_path: Path, kdk_info_plist: Path) -> None:
        ...
        logging.info("Creating backup: {0}".format(kdk_dst_name))
