    ...
    def _pcie_handling(self) -> None:
        ...
        if not self.constants.custom_model and (self.constants.allow_oc_everywhere is True or self.model in model_array.MacPro):
            ...
            for i, controller in enumerate(self.computer.storage):
                logging.info("- Fixing PCIe Storage Controller ({i + 1}) reporting")
                if controller.pci_path:
                    self.config["DeviceProperties"]["Add"][controller.pci_path] = {"built-in": 1}
                else:
                    logging.info("- Failed to find Device path for PCIe Storage Controller {i}, falling back to Innie")
            ...
            if self.constants.allow_nvme_fixing is True:
                for i, controller in enumerate(nvme_devices):
                    if controller.vendor_id == 0x106b:
                        continue
                    logging.info("- Found 3rd Party NVMe SSD ({i}): {vendor_id}:{device_id}".format(i=i+1,vendor_id=utilities.friendly_hex(controller.vendor_id),device_id=utilities.friendly_hex(controller.device_id)))
                    ...
                    if controller.pci_path:
                        logging.info("- Found NVMe ({i}) at {controller_pci_path}".format(i=i,controller_pci_path=controller.pci_path))
