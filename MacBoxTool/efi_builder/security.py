...
class BuildSecurity:
    ...
    def __init__(self, model: str, global_constants: constants.Constants, config: dict) -> None:
        ...
        self.trans = self.translator.security()
    ...
    def _build(self) -> None:
        ...
        if self.constants.sip_status is False or self.constants.custom_sip_value:
            ...
            logging.info(self.trans["- Adding ipc_control_port_options=0 to boot-args"])
            ...
            if self.constants.custom_sip_value:
                logging.info(self.trans["- Setting SIP value to: {self.constants.custom_sip_value}"].format(self=self))
                ...
                logging.info(self.trans["- Set SIP to allow Root Volume patching"])
            ...
            logging.info(self.trans["- Allowing FileVault on Root Patched systems"])
            ...
            logging.info(self.trans["- Enabling KC UUID mismatch patch"])
        ...
        if self.constants.disable_cs_lv is True:
            ...
            if self.constants.disable_amfi is True:
                logging.info(self.trans["- Disabling AMFI"])
                self.config["NVRAM"]["Add"]["7C436110-AB2A-4BBB-A880-FE41995C9F82"]["boot-args"] += " amfi=0x80"
            else:
                logging.info(self.trans["- Disabling Library Validation"])
