...
class GenerateKexts:
    ...
    def _download_airportitlwm(self):
        """Download AirportItlwm for all macOS versions."""
        print("Checking AirportItlwm...")
        repo_url = "https://api.github.com/repos/OpenIntelWireless/itlwm"
        latest_release = requests.get(f"{repo_url}/releases/latest", verify=False).json()

        if "tag_name" not in latest_release:
            print(f"  Error: {latest_release.get('message', 'Unknown error')}")
            return
        ...
        for variant_name in versions:
            kext_name = f"AirportItlwm_{variant_name}.kext"
            local_version = self._get_airportitlwm_local_version(variant_name)

            if local_version and packaging.version.parse(remote_version) <= packaging.version.parse(local_version):
                print(f"  {kext_name} is up to date: v{local_version}")
                continue

            # Download URL format
            download_url = f"https://github.com/OpenIntelWireless/itlwm/releases/download/v{remote_version}/AirportItlwm_v{remote_version}_stable_{variant_name}.kext.zip"
            zip_name = f"AirportItlwm_{variant_name}-v{remote_version}.zip"

            print(f"  Downloading {kext_name}: v{remote_version}...")

            # Remove old version
            if local_version:
                old_zip = f"./WiFi/AirportItlwm_{variant_name}-v{local_version}.zip"
                if Path(old_zip).exists():
                    subprocess.run(["/bin/rm", old_zip], check=True)

            # Download and repackage
            with tempfile.TemporaryDirectory() as temp_dir:
                download = requests.get(download_url, verify=False)
                zip_path = f"{temp_dir}/temp.zip"
                with open(zip_path, "wb") as f:
                    f.write(download.content)

                # Unzip
                subprocess.run(["/usr/bin/unzip", "-q", zip_path, "-d", temp_dir], check=True)

                # Rename AirportItlwm.kext to AirportItlwm_{variant}.kext
                old_kext = Path(temp_dir) / "AirportItlwm.kext"
                new_kext = Path(temp_dir) / kext_name
                if old_kext.exists():
                    old_kext.rename(new_kext)

                # Rezip with new name
                subprocess.run(["/usr/bin/zip", "-q", "-r", zip_name, kext_name], cwd=temp_dir, check=True)

                # Move to destination
                subprocess.run(["/bin/mv", f"{temp_dir}/{zip_name}", f"./WiFi/{zip_name}"], check=True)

        # Update constants file
        self._update_constants_file("self.airportitlwm_version", local_version or "2.3.0", remote_version)

    def _get_airportitlwm_local_version(self, variant_name: str) -> str:
        """Get local version of AirportItlwm variant."""
        loose_name_start = f"AirportItlwm_{variant_name}-v"
        loose_name_end = ".zip"
    ...
    def _special_kext_handling(self):
        # Handle AirportItlwm multi-version downloads
        self._download_airportitlwm()
    ...
    def _update_constants_file(self, variable_name, old_version, new_version):
        ...
        constants_file = Path("../../MacBoxTool/constants.py")
